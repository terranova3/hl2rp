...
function PANEL:configureSteps()
	self:addStep(vgui.Create("ixCharacterFaction"))
	self:addStep(vgui.Create("ixCharacterModel"))
	self:addStep(vgui.Create("ixCharacterBiography"))
...
function PANEL:updateModel()
	local faction = ix.faction.indices[self.context.faction]
...
function PANEL:onFinish()
	...
	PLUGIN:createCharacter(self.context)
		:next(function()
			onResponse()
	...
	timer.Create("ixFailedToCreate", 10, 1, function()
...
function PANEL:addStep(step, priority)
	...
	assert(step.isCharCreateStep, "Panel must inherit ixCharacterCreateStep")
