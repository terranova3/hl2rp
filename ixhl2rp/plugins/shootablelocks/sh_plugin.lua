...
PLUGIN.name = "Shootable Locks";
PLUGIN.description = "Adds shooting a door handle to open a door";
...
function PLUGIN:EntityTakeDamage(entity, dmgInfo)
	if (entity:GetClass() == "prop_door_rotating" and (entity.NextBreach or 0) < CurTime()) then
		...
		if (handle and dmgInfo:IsBulletDamage()) then
			...
			if (!client:IsPlayer() or client:GetEyeTrace().Entity != entity or client:GetPos():Distance(position) > SHOOT_DISTANCE) then
				return
			end
